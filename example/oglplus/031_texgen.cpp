/**
 *  example oglplus/011_texgen.cpp
 *
 *  Copyright Matus Chochlik.
 *  Distributed under the Boost Software License, Version 1.0.
 *  See accompanying file LICENSE_1_0.txt or copy at
 *   http://www.boost.org/LICENSE_1_0.txt
 */

#include <oglplus/gl_fixed.hpp>
#include <oglplus/constants.hpp>
#include <oglplus/operations.hpp>

#include <oglplus/texgen/render_graph.hpp>
#include <oglplus/texgen/sphere_coord_node.hpp>
#include <oglplus/texgen/random_node.hpp>
#include <oglplus/texgen/map_node.hpp>
#include <oglplus/texgen/wrap_node.hpp>
#include <oglplus/texgen/glsl_node.hpp>
#include <oglplus/texgen/border2d_node.hpp>
#include <oglplus/texgen/voronoi2d_node.hpp>
#include <oglplus/texgen/stripes_node.hpp>
#include <oglplus/texgen/combine_node.hpp>
#include <oglplus/texgen/uniform_node.hpp>
#include <oglplus/texgen/offset_node.hpp>
#include <oglplus/texgen/render_node.hpp>

#include "example.hpp"

namespace oglplus {
namespace texgen {

} // namespace texgen

class example_render_graph
 : public texgen::render_graph
{
public:
	uniform<GLfloat> offset_loc;
	uniform<GLfloat> scale_loc;

	example_render_graph(void)
	{
		using namespace texgen;

		add_new_node<sphere_coord_node>("sphere_coords");
		add_new_node<glsl_node>("circle_coords")
			.set_type(slot_data_type::float_2)
			.set_glsl("vec2(${Input}.x, sqrt(${Input}.w))");
		connect("sphere_coords", "circle_coords");

		add_new_node<random_node>("noise");
		add_new_node<wrap_node>("coord_wrap")
			.set_mode(wrap_mode::repeat);

		add_new_node<map_node>("noise_map");
		connect("noise", "noise_map", 0);
		connect("coord_wrap", "noise_map", 1);

		add_new_node<voronoi2d_node>("cells");
		connect("noise_map", "cells");


		add_new_node<map_node>("pattern_coords");

		connect("cells", 4, "pattern_coords", 0);
		connect("circle_coords", "pattern_coords", 1);

		add_new_node<offset_node>("offset");

		uniform_node& uoffset =
		add_new_node<uniform_node>("uoffset").
			set_value_type<float[2]>();

		connect("uoffset", "offset", 1);

		add_new_node<stripes_node>("stripes");
		add_new_node<map_node>("stripes_map");

		connect("stripes", "stripes_map", 0);
		connect("pattern_coords", "stripes_map", 1);

		add_new_node<border2d_node>("stripe_borders");
		connect("stripes_map", "stripe_borders");

		add_new_node<border2d_node>("cell_borders");
		connect("pattern_coords", "cell_borders", 0);

		add_new_node<glsl_node>("cell_edges")
			.set_type(slot_data_type::float_)
			.set_glsl("sign(${Input}.g)");
		connect("cell_borders", "cell_edges");

		add_new_node<combine_node>("combine");
		connect("stripe_borders", "combine", 0);
		connect("cell_edges", "combine", 1);
		connect("stripes_map", "combine", 2);

		add_new_node<glsl_node>("final")
			.set_type(slot_data_type::float_3)
			.set_glsl("vec3(${^}.x+${^}.y*0.2+${^}.z*0.3)");

		connect("combine", "final");
		connect("final", "offset");
		connect_to_renderer("offset");

		finalize();

		offset_loc = uoffset.get_uniform_location();
	}
};

class example_texgen
 : public example
{
private:
	constants  GL;
	operations gl;

	example_render_graph erg;

	GLfloat offset_x, offset_y;

	static constexpr const float min_scale = 0.00001f;
public:
	example_texgen(void)
	 : offset_x(-0.25f)
	 , offset_y(-0.25f)
	{
		gl.uniform(erg.offset_loc, offset_x, offset_y);
		gl.disable(GL.depth_test);
	}

	void pointer_motion(const example_state_view& state)
	override
	{
		if(state.pointer_dragging())
		{
			offset_x -= state.norm_delta_pointer_x();
			offset_y -= state.norm_delta_pointer_y();
			gl.uniform(erg.offset_loc, offset_x, offset_y);
		}
	}

	void resize(const example_state_view& state)
	override
	{
		gl.viewport(0, 0, state.width(), state.height());
		erg.set_dimensions(state.width(), state.height());
	}

	void user_idle(const example_state_view& state)
	override
	{
		if(state.user_idle_time() > seconds_(1))
		{
			const float s = value(state.frame_duration())*60;
			const float t = value(state.exec_time());
			const float r = float(1.1 + std::sin(t*1.618));

			const float dest_offset_x = float(std::sin(t)*r);
			const float dest_offset_y = float(std::cos(t)*r);
			const float c = 0.02f * s;

			offset_x = c*dest_offset_x + (1-c)*offset_x;
			offset_y = c*dest_offset_y + (1-c)*offset_y; 
			gl.uniform(erg.offset_loc, offset_x, offset_y);
		}
	}

	bool continue_running(const example_state_view& state)
	override
	{
		return state.user_idle_time() < seconds_(20);
	}

	void render(const example_state_view& /*state*/)
	override
	{
		erg.render();
	}
};

std::unique_ptr<example>
make_example(
	const example_args&,
	const example_params&,
	const example_state_view&
)
{
	return std::unique_ptr<example>(new example_texgen());
}

void adjust_params(example_params& params)
{
	params.depth_buffer(false);
	params.stencil_buffer(false);
}

bool is_example_param(const example_arg&) { return false; }

} // namespace oglplus
