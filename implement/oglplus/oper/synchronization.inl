...
//------------------------------------------------------------------------------
inline outcome<sync_object> synchronization::fence_sync(
  sync_condition cond) noexcept {
...
//------------------------------------------------------------------------------
inline outcome<sync_status> synchronization::get_sync_status(
  sync_object sync) noexcept {
    return return_sync_i<sync_status, GLenum>(
      sync, sync_parameter(GL_SYNC_STATUS));
}
//------------------------------------------------------------------------------
inline outcome<boolean> synchronization::is_sync_signaled(
  sync_object sync) noexcept {
    GLint result = 0;
