...
struct prog_var_get_set_ops<tag::vertex_attrib, GLbyte> {
    ...
    template <span_size_t N>
    static outcome<void> set(identity<GLbyte[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLbyte, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLbyte[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLbyte, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setN(identity<GLbyte[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLbyte, N>& v) noexcept {
	assert(v.size() >= 4);
...
struct prog_var_get_set_ops<tag::vertex_attrib, GLubyte> {
    ...
    template <span_size_t N>
    static outcome<void> set(identity<GLubyte[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLubyte, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLubyte[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLubyte, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setN(identity<GLubyte[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLubyte, N>& v) noexcept {
	assert(v.size() >= 4);
...
struct prog_var_get_set_ops<tag::vertex_attrib, GLshort> {
    ...
    static outcome<void> set(identity<GLshort>,
      vertex_attrib_location va,
      bool ge,
      GLshort v0) noexcept {
    ...
    template <span_size_t N>
    static outcome<void> set(identity<GLshort[1]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLshort, N>& v) noexcept {
	assert(v.size() >= 1);
    ...
    template <span_size_t N>
    static outcome<void> set(identity<GLshort[2]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLshort, N>& v) noexcept {
	assert(v.size() >= 2);
    ...
    template <span_size_t N>
    static outcome<void> set(identity<GLshort[3]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLshort, N>& v) noexcept {
	assert(v.size() >= 3);
    ...
    template <span_size_t N>
    static outcome<void> set(identity<GLshort[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLshort, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLshort[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLshort, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setN(identity<GLshort[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLshort, N>& v) noexcept {
	assert(v.size() >= 4);
...
struct prog_var_get_set_ops<tag::vertex_attrib, GLushort> {
    ...
    template <span_size_t N>
    static outcome<void> set(identity<GLushort[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLushort, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLushort[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLushort, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setN(identity<GLushort[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLushort, N>& v) noexcept {
	assert(v.size() >= 4);
...
struct prog_var_get_set_ops<tag::vertex_attrib, GLint> {
    ...
    static outcome<void> set(identity<GLint[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLint, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLint[1]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLint, N>& v) noexcept {
	assert(v.size() >= 1);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLint[2]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLint, N>& v) noexcept {
	assert(v.size() >= 2);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLint[3]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLint, N>& v) noexcept {
	assert(v.size() >= 3);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLint[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLint, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setN(identity<GLint[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLint, N>& v) noexcept {
	assert(v.size() >= 4);
...
struct prog_var_get_set_ops<tag::vertex_attrib, GLuint> {
    ...
    static outcome<void> set(identity<GLuint[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLuint, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLuint[1]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLuint, N>& v) noexcept {
	assert(v.size() >= 1);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLuint[2]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLuint, N>& v) noexcept {
	assert(v.size() >= 2);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLuint[3]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLuint, N>& v) noexcept {
	assert(v.size() >= 3);
    ...
    template <span_size_t N>
    static outcome<void> setI(identity<GLuint[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLuint, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    template <span_size_t N>
    static outcome<void> setN(identity<GLuint[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLuint, N>& v) noexcept {
	assert(v.size() >= 4);
    ...
    static outcome<void> setP1(identity<GLuint>,
      vertex_attrib_location va,
      bool ge,
      data_type type,
      boolean norm,
    ...
    static outcome<void> setP2(identity<GLuint>,
      vertex_attrib_location va,
      bool ge,
      data_type type,
      boolean norm,
    ...
    static outcome<void> setP3(identity<GLuint>,
      vertex_attrib_location va,
      bool ge,
      data_type type,
      boolean norm,
    ...
    static outcome<void> setP4(identity<GLuint>,
      vertex_attrib_location va,
      bool ge,
      data_type type,
      boolean norm,
...
struct prog_var_get_set_ops<tag::vertex_attrib, GLdouble> {
    ...
    static outcome<void> setL(identity<GLdouble>,
      vertex_attrib_location va,
      bool ge,
      GLdouble v0) noexcept {
    ...
    template <span_size_t N>
    static outcome<void> setL(identity<GLdouble[1]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLdouble, N>& v) noexcept {
	assert(v.size() >= 1);
    ...
    template <span_size_t N>
    static outcome<void> setL(identity<GLdouble[2]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLdouble, N>& v) noexcept {
	assert(v.size() >= 2);
    ...
    template <span_size_t N>
    static outcome<void> setL(identity<GLdouble[3]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLdouble, N>& v) noexcept {
	assert(v.size() >= 3);
    ...
    template <span_size_t N>
    static outcome<void> setL(identity<GLdouble[4]>,
      vertex_attrib_location va,
      bool ge,
      const span<const GLdouble, N>& v) noexcept {
	assert(v.size() >= 4);
