...
outcome<void> initialize_vao_and_buffers(
  ...
  eagine::memory::buffer& data) {
    ...
    while(vaali != vaale) {
        ...
        if(
          auto res = failure(gl.buffer_data(
            GL.array_buffer, buffer_data_spec(data), GL.static_draw))) {
            return std::move(res);
        }

        if(
          auto res = failure(gl.vertex_array_attrib_pointer(
            location,
    ...
    if(gen.indexed()) {
        ...
        if(
          auto res = failure(gl.buffer_data(
            GL.element_array_buffer, buffer_data_spec(data), GL.static_draw))) {
            return std::move(res);
        }
    }
