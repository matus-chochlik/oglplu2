...
//------------------------------------------------------------------------------
inline outcome<shader_type>
shader_ops::get_shader_type(shader_name shdr) noexcept {
...
shader_ops::get_shader_delete_status(shader_name shdr) noexcept {
    return return_shader_i<boolean, GLboolean>(
      shdr, shader_parameter(GL_DELETE_STATUS));
}
//------------------------------------------------------------------------------
inline outcome<boolean>
shader_ops::get_shader_compile_status(shader_name shdr) noexcept {
    return return_shader_i<boolean, GLboolean>(
      shdr, shader_parameter(GL_COMPILE_STATUS));
}
//------------------------------------------------------------------------------
inline outcome<GLsizei>
shader_ops::get_shader_source_length(shader_name shdr) noexcept {
    return return_shader_i<GLsizei, GLsizei>(
      shdr, shader_parameter(GL_SHADER_SOURCE_LENGTH));
}
//------------------------------------------------------------------------------
inline outcome<span<char>>
shader_ops::get_shader_source(shader_name shdr, span<char> dest) noexcept {
    GLsizei reallen = 0;
...
//------------------------------------------------------------------------------
inline outcome<GLsizei>
shader_ops::get_shader_info_log_length(shader_name shdr) noexcept {
    return return_shader_i<GLsizei, GLsizei>(
      shdr, shader_parameter(GL_INFO_LOG_LENGTH));
}
//------------------------------------------------------------------------------
inline outcome<span<char>>
shader_ops::get_shader_info_log(shader_name shdr, span<char> dest) noexcept {
    GLsizei reallen = 0;
...
//------------------------------------------------------------------------------
inline outcome<boolean>
obj_gen_del_ops<tag::shader>::_is_a(GLuint name) noexcept {
