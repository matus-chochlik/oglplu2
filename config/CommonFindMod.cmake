...
#
macro(eagine_common_find_module PREFIX PC_NAME HEADER LIBRARY)
	unset(${PREFIX}_FOUND)
	...
	if((${PREFIX}_INCLUDE_DIRS) AND (${PREFIX}_LIBRARIES))
		...
		if(EXISTS ${PROJECT_SOURCE_DIR}/config/ext_lib/test_${PREFIX}.cpp)
			...
			try_run(
				...
				"${PROJECT_BINARY_DIR}/ext_lib/test_${PREFIX}.cpp"
				COMPILE_DEFINITIONS
					"${CPP_STD_COMPILER_SWITCH} ${${PREFIX}_DEFINITIONS}"
				CMAKE_FLAGS
		...
		set(FAIL_MSG "Could NOT find ${PREFIX}")
		if(NOT(${PREFIX}_INCLUDE_DIRS) OR NOT(${PREFIX}_LIBRARIES))
			set(FAIL_MSG "${FAIL_MSG} (missing:")
			if(NOT(${PREFIX}_INCLUDE_DIRS))
				set(FAIL_MSG "${FAIL_MSG} ${PREFIX}_INCLUDE_DIRS")
			endif()
			if(NOT(${PREFIX}_LIBRARIES))
				set(FAIL_MSG "${FAIL_MSG} ${PREFIX}_LIBRARIES")
			endif()
