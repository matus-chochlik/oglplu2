//  File test/boost_test/oalplus/enums/buffer_parameter.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_buffer_parameter_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "BITS") == 0);
#endif

#ifdef AL_CHANNELS
    x = ev.channels;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "CHANNELS") == 0);
#endif
    ...
    x = ev.size;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "SIZE") == 0);
#endif
}
