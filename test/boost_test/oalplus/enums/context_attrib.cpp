//  File test/boost_test/oalplus/enums/context_attrib.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_context_attrib_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "FREQUENCY") == 0);
#endif

#ifdef ALC_MONO_SOURCES
    x = ev.mono_sources;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "MONO_SOURCES") == 0);
#endif

#ifdef ALC_REFRESH
    x = ev.refresh;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "REFRESH") == 0);
#endif

#ifdef ALC_STEREO_SOURCES
    x = ev.stereo_sources;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "STEREO_SOURCES") == 0);
#endif

#ifdef ALC_SYNC
    x = ev.sync;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "SYNC") == 0);
#endif
}
