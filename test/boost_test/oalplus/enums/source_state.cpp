//  File test/boost_test/oalplus/enums/source_state.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_source_state_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "INITIAL") == 0);
#endif

#ifdef AL_PAUSED
    x = ev.paused;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "PAUSED") == 0);
#endif

#ifdef AL_PLAYING
    x = ev.playing;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "PLAYING") == 0);
#endif

#ifdef AL_STOPPED
    x = ev.stopped;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "STOPPED") == 0);
#endif
}
