//  File test/boost_test/oalplus/enums/data_format.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_data_format_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "FORMAT_MONO16") == 0);
#endif

#ifdef AL_FORMAT_MONO8
    x = ev.format_mono8;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "FORMAT_MONO8") == 0);
#endif

#ifdef AL_FORMAT_STEREO16
    x = ev.format_stereo16;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "FORMAT_STEREO16") == 0);
#endif

#ifdef AL_FORMAT_STEREO8
    x = ev.format_stereo8;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "FORMAT_STEREO8") == 0);
#endif
}
