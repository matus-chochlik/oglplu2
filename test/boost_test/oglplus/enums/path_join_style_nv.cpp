//  File test/boost_test/oglplus/enums/path_join_style_nv.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_path_join_style_nv_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "BEVEL_NV") == 0);
#endif

#ifdef GL_MITER_REVERT_NV
    x = ev.miter_revert_nv;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "MITER_REVERT_NV") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "MITER_TRUNCATE_NV") == 0);
#endif
    ...
    x = ev.round_nv;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "ROUND_NV") == 0);
#endif
}
