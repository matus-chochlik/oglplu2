//  File test/boost_test/oglplus/enums/path_cap_style_nv.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_path_cap_style_nv_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "FLAT") == 0);
#endif
    ...
    x = ev.square_nv;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "SQUARE_NV") == 0);
#endif

#ifdef GL_TRIANGULAR_NV
    x = ev.triangular_nv;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "TRIANGULAR_NV") == 0);
#endif
}
