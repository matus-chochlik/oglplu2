//  File test/boost_test/oglplus/enums/framebuffer_target.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_framebuffer_target_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "DRAW_FRAMEBUFFER") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "READ_FRAMEBUFFER") == 0);
#endif
}
...
BOOST_AUTO_TEST_CASE(enum_framebuffer_target_range) {
	...
	BOOST_CHECK(
	  std::find(r.begin(), r.end(), framebuffer_target(GL_DRAW_FRAMEBUFFER))
	  != r.end());
    }
#endif
	...
	auto r = enum_value_range<framebuffer_target>();
	BOOST_CHECK(
	  std::find(r.begin(), r.end(), framebuffer_target(GL_READ_FRAMEBUFFER))
	  != r.end());
    }
#endif
