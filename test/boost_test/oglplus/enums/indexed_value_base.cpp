//  File test/boost_test/oglplus/enums/indexed_value_base.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_indexed_value_base_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "CLIP_DISTANCE0") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "COLOR_ATTACHMENT0") == 0);
#endif

#ifdef GL_DRAW_BUFFER0
    x = ev.draw_buffer0;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "DRAW_BUFFER0") == 0);
#endif

#ifdef GL_TEXTURE0
    x = ev.texture0;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "TEXTURE0") == 0);
#endif
}
