//  File test/boost_test/oglplus/enums/patch_parameter.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_patch_parameter_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "PATCH_DEFAULT_INNER_LEVEL") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "PATCH_DEFAULT_OUTER_LEVEL") == 0);
#endif

#ifdef GL_PATCH_VERTICES
    x = ev.patch_vertices;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "PATCH_VERTICES") == 0);
#endif
}
