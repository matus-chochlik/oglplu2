//  File test/boost_test/oglplus/enums/query_parameter.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_query_parameter_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "QUERY_RESULT") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "QUERY_RESULT_AVAILABLE") == 0);
#endif
}
