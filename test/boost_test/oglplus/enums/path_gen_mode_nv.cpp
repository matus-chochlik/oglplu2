//  File test/boost_test/oglplus/enums/path_gen_mode_nv.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_path_gen_mode_nv_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "EYE_LINEAR") == 0);
#endif
    ...
    x = ev.object_linear;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "OBJECT_LINEAR") == 0);
#endif
