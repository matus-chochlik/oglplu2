//  File test/boost_test/oglplus/enums/texture_wrap_mode.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_texture_wrap_mode_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "CLAMP_TO_BORDER") == 0);
#endif

#ifdef GL_CLAMP_TO_EDGE
    x = ev.clamp_to_edge;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "CLAMP_TO_EDGE") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "MIRROR_CLAMP_TO_EDGE") == 0);
#endif

#ifdef GL_MIRRORED_REPEAT
    x = ev.mirrored_repeat;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "MIRRORED_REPEAT") == 0);
#endif

#ifdef GL_REPEAT
    x = ev.repeat;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "REPEAT") == 0);
#endif
}
