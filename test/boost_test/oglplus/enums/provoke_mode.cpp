//  File test/boost_test/oglplus/enums/provoke_mode.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_provoke_mode_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "FIRST_VERTEX_CONVENTION") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "LAST_VERTEX_CONVENTION") == 0);
#endif
}
