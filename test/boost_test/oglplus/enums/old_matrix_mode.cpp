//  File test/boost_test/oglplus/enums/old_matrix_mode.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_old_matrix_mode_names) {
    ...
    x = ev.modelview;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "MODELVIEW") == 0);
#endif

#ifdef GL_PROJECTION
    x = ev.projection;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "PROJECTION") == 0);
#endif
