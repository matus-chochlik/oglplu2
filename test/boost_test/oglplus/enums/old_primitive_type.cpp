//  File test/boost_test/oglplus/enums/old_primitive_type.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_old_primitive_type_names) {
    ...
    x = ev.polygon;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "POLYGON") == 0);
#endif

#ifdef GL_QUAD_STRIP
    x = ev.quad_strip;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "QUAD_STRIP") == 0);
#endif
