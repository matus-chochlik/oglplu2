//  File test/boost_test/oglplus/enums/clip_depth_mode.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_clip_depth_mode_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "NEGATIVE_ONE_TO_ONE") == 0);
#endif

#ifdef GL_ZERO_TO_ONE
    x = ev.zero_to_one;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "ZERO_TO_ONE") == 0);
#endif
}
