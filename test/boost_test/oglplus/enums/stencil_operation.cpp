//  File test/boost_test/oglplus/enums/stencil_operation.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_stencil_operation_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "DECR") == 0);
#endif

#ifdef GL_DECR_WRAP
    x = ev.decr_wrap;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "DECR_WRAP") == 0);
#endif

#ifdef GL_INCR
    x = ev.incr;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "INCR") == 0);
#endif

#ifdef GL_INCR_WRAP
    x = ev.incr_wrap;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "INCR_WRAP") == 0);
#endif
    ...
    x = ev.keep;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "KEEP") == 0);
#endif

#ifdef GL_REPLACE
    x = ev.replace;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "REPLACE") == 0);
#endif
