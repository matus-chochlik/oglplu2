//  File test/boost_test/oglplus/enums/sync_status.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_sync_status_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "SIGNALED") == 0);
#endif

#ifdef GL_UNSIGNALED
    x = ev.unsignaled;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "UNSIGNALED") == 0);
#endif
}
