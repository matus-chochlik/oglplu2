//  File test/boost_test/oglplus/enums/sync_wait_result.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_sync_wait_result_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "ALREADY_SIGNALED") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "CONDITION_SATISFIED") == 0);
#endif

#ifdef GL_TIMEOUT_EXPIRED
    x = ev.timeout_expired;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "TIMEOUT_EXPIRED") == 0);
#endif

#ifdef GL_WAIT_FAILED
    x = ev.wait_failed;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "WAIT_FAILED") == 0);
#endif
}
