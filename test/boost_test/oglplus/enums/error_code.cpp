//  File test/boost_test/oglplus/enums/error_code.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_error_code_names) {
    ...
    x = ev.stack_overflow;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "STACK_OVERFLOW") == 0);
#endif

#ifdef GL_STACK_UNDERFLOW
    x = ev.stack_underflow;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "STACK_UNDERFLOW") == 0);
#endif

#ifdef GL_TABLE_TOO_LARGE
    x = ev.table_too_large;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "TABLE_TOO_LARGE") == 0);
#endif
}
