//  File test/boost_test/oglplus/enums/transform_feedback_parameter.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_transform_feedback_parameter_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "TRANSFORM_FEEDBACK_ACTIVE") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "TRANSFORM_FEEDBACK_PAUSED") == 0);
#endif
}
