//  File test/boost_test/oglplus/enums/precision_type.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_precision_type_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "HIGH_FLOAT") == 0);
#endif

#ifdef GL_HIGH_INT
    x = ev.high_int;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "HIGH_INT") == 0);
#endif

#ifdef GL_LOW_FLOAT
    x = ev.low_float;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "LOW_FLOAT") == 0);
#endif

#ifdef GL_LOW_INT
    x = ev.low_int;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "LOW_INT") == 0);
#endif

#ifdef GL_MEDIUM_FLOAT
    x = ev.medium_float;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "MEDIUM_FLOAT") == 0);
#endif

#ifdef GL_MEDIUM_INT
    x = ev.medium_int;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "MEDIUM_INT") == 0);
#endif
}
