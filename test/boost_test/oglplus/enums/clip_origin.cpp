//  File test/boost_test/oglplus/enums/clip_origin.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_clip_origin_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "LOWER_LEFT") == 0);
#endif

#ifdef GL_UPPER_LEFT
    x = ev.upper_left;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "UPPER_LEFT") == 0);
#endif
}
...
BOOST_AUTO_TEST_CASE(enum_clip_origin_range) {
...
#ifdef GL_LOWER_LEFT
    {
        ...
        BOOST_CHECK(
          std::find(r.begin(), r.end(), clip_origin(GL_LOWER_LEFT)) != r.end());
    }
#endif
...
#ifdef GL_UPPER_LEFT
    {
        ...
        auto r = enum_value_range<clip_origin>();
        BOOST_CHECK(
          std::find(r.begin(), r.end(), clip_origin(GL_UPPER_LEFT)) != r.end());
    }
#endif
