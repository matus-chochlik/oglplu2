//  File test/boost_test/oglplus/enums/compare_function.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_compare_function_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "ALWAYS") == 0);
#endif
    ...
    x = ev.gequal;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "GEQUAL") == 0);
#endif

#ifdef GL_GREATER
    x = ev.greater;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "GREATER") == 0);
#endif

#ifdef GL_LEQUAL
    x = ev.lequal;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "LEQUAL") == 0);
#endif

#ifdef GL_LESS
    x = ev.less;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "LESS") == 0);
#endif

#ifdef GL_NEVER
    x = ev.never;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "NEVER") == 0);
#endif

#ifdef GL_NOTEQUAL
    x = ev.notequal;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "NOTEQUAL") == 0);
#endif
}
