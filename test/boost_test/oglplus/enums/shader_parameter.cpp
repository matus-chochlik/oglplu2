//  File test/boost_test/oglplus/enums/shader_parameter.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_shader_parameter_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "COMPILE_STATUS") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "SHADER_SOURCE_LENGTH") == 0);
#endif

#ifdef GL_SHADER_TYPE
    x = ev.shader_type;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "SHADER_TYPE") == 0);
#endif
}
