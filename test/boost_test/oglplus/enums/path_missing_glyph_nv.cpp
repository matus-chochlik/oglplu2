...
BOOST_AUTO_TEST_CASE(enum_path_missing_glyph_nv_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "SKIP_MISSING_GLYPH_NV") == 0);
#endif
    ...
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "USE_MISSING_GLYPH_NV") == 0);
#endif
}
...
BOOST_AUTO_TEST_CASE(enum_path_missing_glyph_nv_range) {
	...
	BOOST_CHECK(
	  std::find(
	    r.begin(), r.end(), path_missing_glyph_nv(GL_SKIP_MISSING_GLYPH_NV))
	  != r.end());
    }
#endif
	...
	BOOST_CHECK(
	  std::find(
	    r.begin(), r.end(), path_missing_glyph_nv(GL_USE_MISSING_GLYPH_NV))
	  != r.end());
    }
#endif
