...
BOOST_AUTO_TEST_CASE(enum_sync_parameter_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "OBJECT_TYPE") == 0);
#endif

#ifdef GL_SYNC_CONDITION
    x = ev.sync_condition;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "SYNC_CONDITION") == 0);
#endif
    ...
    x = ev.sync_status;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "SYNC_STATUS") == 0);
#endif
}
