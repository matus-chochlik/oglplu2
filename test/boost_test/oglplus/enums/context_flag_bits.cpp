//  File test/boost_test/oglplus/enums/context_flag_bits.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_context_flag_bits_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "CONTEXT_FLAG_DEBUG_BIT") == 0);
#endif
    ...
    BOOST_CHECK(
      std::strcmp(
        enum_value_name(x).data(), "CONTEXT_FLAG_FORWARD_COMPATIBLE_BIT") == 0);
#endif
    ...
    BOOST_CHECK(
      std::strcmp(
        enum_value_name(x).data(), "CONTEXT_FLAG_ROBUST_ACCESS_BIT_ARB") == 0);
#endif
}
...
BOOST_AUTO_TEST_CASE(enum_context_flag_bits_range) {
...
#ifdef GL_CONTEXT_FLAG_DEBUG_BIT
    {
        ...
        BOOST_CHECK(
          std::find(
            r.begin(), r.end(), context_flag_bits(GL_CONTEXT_FLAG_DEBUG_BIT)) !=
          r.end());
    }
#endif
