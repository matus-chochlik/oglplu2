//  File test/boost_test/oglplus/enums/access_specifier.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_access_specifier_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "READ_ONLY") == 0);
#endif

#ifdef GL_READ_WRITE
    x = ev.read_write;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "READ_WRITE") == 0);
#endif

#ifdef GL_WRITE_ONLY
    x = ev.write_only;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "WRITE_ONLY") == 0);
#endif
}
