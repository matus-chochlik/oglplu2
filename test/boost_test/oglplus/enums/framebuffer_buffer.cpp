//  File test/boost_test/oglplus/enums/framebuffer_buffer.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_framebuffer_buffer_names) {
    ...
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "COLOR") == 0);
#endif

#ifdef GL_DEPTH
    x = ev.depth;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "DEPTH") == 0);
#endif
    ...
    x = ev.stencil;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "STENCIL") == 0);
#endif
}
