...
BOOST_AUTO_TEST_CASE(error_handling_1) {
	...
	BOOST_CHECK_MESSAGE(false, "Should not get past handle_gl_error");
    } catch(oglplus::error& e) {
	BOOST_CHECK(e.info().gl_error_code() == GL_OUT_OF_MEMORY);

	BOOST_CHECK(((e.info().gl_library_name() == nullptr)
	...
	BOOST_CHECK(((e.info().source_function() == nullptr)
		     || (strcmp(e.info().source_function(), "function") == 0)));

	BOOST_CHECK(((e.info().source_file() == nullptr)
		     || (strcmp(e.info().source_file(), "file.cpp") == 0)));

	BOOST_CHECK(
	  ((e.info().source_line() == 0) || (e.info().source_line() == 12345)));

	BOOST_CHECK(((e.info().gl_object() == mock_object_name())
		     || (e.info().gl_object(), mock_object_name(23456))));

	BOOST_CHECK(((e.info().gl_subject() == mock_object_name())
...
BOOST_AUTO_TEST_CASE(error_handling_2) {
	...
	BOOST_CHECK(((e.info().gl_function_name() == nullptr)
	...
	BOOST_CHECK(
	  ((e.info().source_line() == 0) || (e.info().source_line() == 54321)));
...
BOOST_AUTO_TEST_CASE(error_handling_3) {
	...
	BOOST_CHECK(
	  ((e.info().source_line() == 0) || (e.info().source_line() == 11111)));
