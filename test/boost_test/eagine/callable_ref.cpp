...
BOOST_AUTO_TEST_CASE(callable_ref_foo_2) {
    ...
    BOOST_CHECK_EQUAL(
      cr2(234.5f, 345.6f, 456.7f), bar::foo(&br, 234.5f, 345.6f, 456.7f));
}
...
BOOST_AUTO_TEST_CASE(callable_ref_foo_3) {
    ...
    BOOST_CHECK_EQUAL(
      cr3(345.6f, 456.7f, 567.8f), bar::foo(&br, 345.6f, 456.7f, 567.8f));
}
...
BOOST_AUTO_TEST_CASE(callable_ref_foo_5) {
    ...
    BOOST_CHECK_EQUAL(
      cr5(567.8f, 678.9f, 789.0f), br.foo(567.8f, 678.9f, 789.0f));
}
...
BOOST_AUTO_TEST_CASE(callable_ref_baz_2) {
    ...
    callable_ref<baz&(int)> cr(
      &bz, member_function_constant<baz& (baz::*)(int), &baz::add>());
