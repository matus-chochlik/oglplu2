...
BOOST_AUTO_TEST_CASE(string_list_elem_3)
{
	...
	const char s3[132] = {char(0xC2),char(0x80),
		'x','x','x','x','x','x','x','x',
		'x','x','x','x','x','x','x','x',
		'x','x','x','x','x','x','x','x',
		'x','x','x','x','x','x','x','x',
...
BOOST_AUTO_TEST_CASE(string_list_for_each_elem_1)
{
	...
	int i=1;
	string_list::for_each_elem(ss1,
		[&i](string_list::element e, bool first) {
...
BOOST_AUTO_TEST_CASE(string_list_rev_for_each_elem_1)
{
	...
	int i=5;
	string_list::rev_for_each_elem(ss1,
		[&i](string_list::element e, bool first) {
...
BOOST_AUTO_TEST_CASE(string_list_for_each_1)
{
	...
	string_list::for_each(ss1,
		[&ts](cstring_span s) {
			ts.append(s.data(), std_size(s.size()));
		}
	);
	BOOST_CHECK_EQUAL(ts, "aBcdEfGhIjkLmNo");
}
