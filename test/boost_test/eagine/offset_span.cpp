 ...
 */
#include <eagine/offset_span.hpp>
...
BOOST_AUTO_TEST_CASE(offset_span_2) {
    ...
    offset_span<double>::size_type i = 0;
    for(double d : ad) {
...
BOOST_AUTO_TEST_CASE(offset_span_3) {
    ...
    offset_span<const char> acc(cstr, span_size(std::strlen(cstr)));
...
BOOST_AUTO_TEST_CASE(offset_span_4) {
    ...
    const offset_span<int> cai(ia, 20);
...
BOOST_AUTO_TEST_CASE(offset_span_5) {
    ...
    offset_span<const char> acc(str);
    ...
    offset_span<const char>::size_type i = 0;
    for(char c : acc) {
