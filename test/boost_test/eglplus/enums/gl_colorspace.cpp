//  File test/boost_test/eglplus/enums/gl_colorspace.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_gl_colorspace_range) {
	...
	BOOST_CHECK(
	  std::find(r.begin(), r.end(), gl_colorspace(EGL_GL_COLORSPACE_LINEAR))
	  != r.end());
    }
#endif
	...
	auto r = enum_value_range<gl_colorspace>();
	BOOST_CHECK(
	  std::find(r.begin(), r.end(), gl_colorspace(EGL_GL_COLORSPACE_sRGB))
	  != r.end());
    }
#endif
