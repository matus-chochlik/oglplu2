//  File test/boost_test/eglplus/enums/rendering_api.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_rendering_api_range) {
	...
	--count;
	auto r = enum_value_range<rendering_api>();
	BOOST_CHECK(std::find(r.begin(), r.end(), rendering_api(EGL_OPENGL_API))
		    != r.end());
    }
#endif
	...
	--count;
	auto r = enum_value_range<rendering_api>();
	BOOST_CHECK(std::find(r.begin(), r.end(), rendering_api(EGL_OPENVG_API))
		    != r.end());
    }
#endif
