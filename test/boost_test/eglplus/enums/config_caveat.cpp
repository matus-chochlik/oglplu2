//  File test/boost_test/eglplus/enums/config_caveat.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_config_caveat_names) {
    ...
    BOOST_CHECK(
      std::strcmp(enum_value_name(x).data(), "NON_CONFORMANT_CONFIG") == 0);
#endif
    ...
    x = ev.slow_config;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "SLOW_CONFIG") == 0);
#endif
}
...
BOOST_AUTO_TEST_CASE(enum_config_caveat_range) {
...
#ifdef EGL_NON_CONFORMANT_CONFIG
    {
        ...
        BOOST_CHECK(
          std::find(
            r.begin(), r.end(), config_caveat(EGL_NON_CONFORMANT_CONFIG)) !=
          r.end());
    }
#endif
...
#ifdef EGL_NONE
    {
        ...
        auto r = enum_value_range<config_caveat>();
        BOOST_CHECK(
          std::find(r.begin(), r.end(), config_caveat(EGL_NONE)) != r.end());
    }
#endif
...
#ifdef EGL_SLOW_CONFIG
    {
        ...
        auto r = enum_value_range<config_caveat>();
        BOOST_CHECK(
          std::find(r.begin(), r.end(), config_caveat(EGL_SLOW_CONFIG)) !=
          r.end());
    }
#endif
