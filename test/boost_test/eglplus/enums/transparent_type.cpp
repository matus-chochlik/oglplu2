//  File test/boost_test/eglplus/enums/transparent_type.cpp
//
...
BOOST_AUTO_TEST_CASE(enum_transparent_type_names) {
    ...
    x = ev.transparent_rgb;
    BOOST_ASSERT(enum_value_name(x).data() != nullptr);
    BOOST_CHECK(std::strcmp(enum_value_name(x).data(), "TRANSPARENT_RGB") == 0);
#endif
}
...
BOOST_AUTO_TEST_CASE(enum_transparent_type_range) {
	...
	BOOST_CHECK(
	  std::find(r.begin(), r.end(), transparent_type(EGL_NONE)) != r.end());
    }
#endif
	...
	auto r = enum_value_range<transparent_type>();
	BOOST_CHECK(
	  std::find(r.begin(), r.end(), transparent_type(EGL_TRANSPARENT_RGB))
	  != r.end());
    }
#endif
