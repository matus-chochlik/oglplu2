	...
	for(int r=0; r<100; ++r) {
		switch(rg.get_int(0, 2)) {
			default: {
				...
				BOOST_CHECK_EQUAL(
					std::invoke(&invk_test_3::foo, t, y),
					t.foo(y)
				);
				break;
			} case 1: {
				...
				BOOST_CHECK_EQUAL(
					std::invoke(&invk_test_3::bar, t, y, z),
					t.bar(y, z)
				);
				break;
			} case 2: {
