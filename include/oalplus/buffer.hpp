...
struct buffer_ops {
    ...
    static outcome<void> get_buffer_iv(
      buffer_name buf, buffer_parameter param, span<ALint> values) noexcept;

    static outcome<void> get_buffer_fv(
      buffer_name buf, buffer_parameter param, span<ALfloat> values) noexcept;

    static outcome<ALint> buffer_size(buffer_name buf) noexcept;

    static outcome<ALint> buffer_bits(buffer_name buf) noexcept;

    static outcome<ALint> buffer_channels(buffer_name buf) noexcept;

    static outcome<ALfloat> buffer_frequency(buffer_name buf) noexcept;
};
...
struct obj_gen_del_ops<tag::buffer> {
    static deferred_error_handler _gen(span<ALuint> names) noexcept;

    static deferred_error_handler _delete(span<ALuint> names) noexcept;

    static outcome<bool> _is_a(ALuint name) noexcept;
};
