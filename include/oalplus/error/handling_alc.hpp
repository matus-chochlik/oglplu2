 ...
 */
#ifndef OALPLUS_ERROR_HANDLING_ALC_HPP
#define OALPLUS_ERROR_HANDLING_ALC_HPP
...
struct alc_error_handling_policy {
    static bool is_valid(const error_info_alc& info) noexcept {
        return is_alc_error(info.al_error_code());
    }

    static void invoke(error_info_alc& info) {
        handle_alc_error(info);
    }

    static void cancel(error_info_alc& info) noexcept {
        info.al_error_code(AL_NO_ERROR);
    }
};

using deferred_error_handler_alc =
  deferred_handler<error_info_alc, alc_error_handling_policy>;
...
#define OALPLUS_RETURN_ALC_HANDLER_IF(                                      \
  CONDITION, DEVICE, ERROR_CODE, ERROR_INFO, SEVERITY)                      \
    {                                                                       \
        ...
        if(CONDITION(oalplus_error_code##__LINE__)) {                       \
            return oalplus::deferred_error_handler_alc(std::move(           \
              oalplus::error_info_alc(DEVICE, oalplus_error_code##__LINE__) \
...
#define OALPLUS_RETURN_HANDLER_IF_ALC_ERROR( \
  DEVICE, ERROR_CODE, ERROR_INFO, SEVERITY)  \
...
#endif // OALPLUS_ERROR_HANDLING_ALC_HPP
