...
void handle_al_error(ErrorInfo& info)
{
	throw error(std::move(info));
}

static inline constexpr
bool is_al_error(ALenum ec)
noexcept
{
...
#define OALPLUS_RETURN_HANDLER_IF(\
	CONDITION,\
	ERROR_CODE,\
	ERROR_INFO,\
	SEVERITY\
...
{\
	...
	{\
		return oalplus::deferred_error_handler(std::move(\
			oalplus::error_info(oalplus_error_code##__LINE__)\
				.ERROR_INFO\
...
#define OALPLUS_RETURN_HANDLER_IF_AL_ERROR(\
	ERROR_CODE,\
	ERROR_INFO,\
	SEVERITY\
