...
public:
	...
	noexcept { std::memset(_str, '\0', N); }

	template <
		typename ... C,
		typename = std::enable_if_t<sizeof ... (C) == N>
	>
	constexpr inline
	fixed_size_string(C ... c)
	noexcept
	 : _str{c...}
	{ }

	fixed_size_string(const char (&s)[N])
	noexcept {
	...
	noexcept { return _str[0] == '\0'; }

	span_size_t size(void) const
	noexcept { return N-1; }

	const char* data(void) const
	noexcept { return _str; }
	...
	iterator end(void)
	noexcept { return _str+N-1; }

	const_iterator end(void) const
	noexcept { return _str+N-1; }

	cstr_ref ref(void) const
	noexcept { return cstr_ref(_str, N-1); }

	operator cstr_ref (void) const
	noexcept { return ref(); }
};
...
noexcept { return fixed_size_string<N>(str); }
...
noexcept { return fixed_size_string<N1+N2-1>(s1, s2); }

template <int I>
static inline
auto to_fixed_size_string(
	int_constant<I>,
	std::enable_if_t<(I >= 0) && (I < 10)>* = 0
) noexcept { return fixed_size_string<2>(char('0' + I), '\0'); }

template <int I>
static inline
auto to_fixed_size_string(
	int_constant<I>,
	std::enable_if_t<(I > 9)>* = 0
) noexcept {
...
auto to_fixed_size_string(
	int_constant<I>,
	std::enable_if_t<(I < 0)>* = 0
) noexcept {
	return fixed_size_string<2>("-")+
		to_fixed_size_string(int_constant<-I>());
}
