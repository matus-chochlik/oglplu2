...
struct make_scaled_base_dim_unit<base::scaled_unit<Scale, BaseUnit>, System>
  : scaled_dim_unit<dimension<dimension_of_t<BaseUnit>, 1>,
...
struct value_conv<scaled_dim_unit<D, add_none_unit_scale_t<AS, US>, System>,
  unit<D, System>> {
    template <typename T>
    constexpr inline auto operator()(T v) const {
	return AS::mul(v);
...
template <typename D, typename AS, typename US, typename System>
struct value_conv<unit<D, System>,
...
struct value_conv<scaled_dim_unit<D, add_none_unit_scale_t<AS1, US>, System>,
