...
bool value_with_history_changed(const T& a, const T& b)
noexcept {
#ifdef __clang__
...
template <typename T>
static constexpr inline
auto value_with_history_delta(const T& new_value, const T& old_value)
noexcept {
...
public:
	constexpr inline
	value_with_history_storage(void) = default;

	template <typename ... I>
	constexpr inline
	value_with_history_storage(I&& ... initial)
	 : _values{T(initial)...}
	{ }

	value_with_history_storage(const T& initial)
	noexcept {
	...
	noexcept { return _values[i]; }

	inline
	void set(std::size_t i, const T& value)
	noexcept { _values[i] = value; }

	void make_history(void)
	noexcept {
...
auto convert_stored_values(const value_with_history_storage<T, N>& storage) {
	return transform_stored_values(
		[](const T& v) { return U(v); },
		storage
	);
}
...
protected:
	value_with_history_storage<T, N>& values(void)
	...
	inline explicit
	value_with_history(const T& initial)
	noexcept
	 : _values(initial)
	{ }
public:
	inline explicit
	value_with_history(const value_with_history_storage<T, N>& storage)
	 : _values(storage)
	{ }

	template <typename ... I>
	constexpr inline
	value_with_history(I&& ... initial)
	 : _values{std::forward<I>(initial)...}
	{ }

	const value_with_history_storage<T, N>& values(void) const
	noexcept { return _values; }

	auto get(void) const
	noexcept { return values().get(0); }

	auto value(void) const
	noexcept { return values().get(0); }

	auto old_value(void) const
	noexcept { return values().get(1); }
	...
	operator valid_if<U, P...> (void) const
...
auto transform(
	...
	return value_with_history<
		decltype(std::declval<Transform>()(std::declval<T>()...)),
		N
	>(transform_stored_values(transform_op, v.values()...));
}
...
auto operator * (
	...
	const value_with_history<T2, N>& v2
) noexcept {
	return transform(
...
public:
	constexpr inline
	variable_with_history(const T& initial)
	noexcept
...
public:
	constexpr inline
	variable_with_history(const valid_if<T, P...>& initial)
	noexcept
