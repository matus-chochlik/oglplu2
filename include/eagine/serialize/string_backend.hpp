...
private:
    ...
    auto _write_one(char value, type_identity<char>) -> result {
        result errors = sink('\'');
        errors |= sink(value);
        errors |= sink('\'');
        return errors;
    ...
    auto _write_one(byte value, type_identity<byte>) -> result {
        return _sprintf_one(value, "%02hhx");
    }

    auto _write_one(signed char value, type_identity<signed char>) -> result {
        return _sprintf_one(value, "%hhd");
    }

    auto _write_one(short value, type_identity<short>) -> result {
    ...
    auto _write_one(int value, type_identity<int>) -> result {
        return _sprintf_one(value, "%d");
    }

    auto _write_one(unsigned value, type_identity<unsigned>) -> result {
        return _sprintf_one(value, "%u");
    }

    auto _write_one(long value, type_identity<long>) -> result {
    ...
    auto _write_one(long long value, type_identity<long long>) -> result {
        return _sprintf_one(value, "%lld");
    }

    auto _write_one(unsigned long long value, type_identity<unsigned long long>)
      -> result {
        return _sprintf_one(value, "%llu");
    }

    auto _write_one(float value, type_identity<float>) -> result {
        return _sprintf_one(value, "%f");
    }

    auto _write_one(double value, type_identity<double>) -> result {
...
private:
    ...
    auto _read_one(char& value, char delimiter) -> result {
        result errors = require('\'');
        if(!errors) {
    ...
    auto _read_one(byte& value, char delimiter) -> result {
        const char fmt[6] = {'%', 'h', 'h', 'x', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(signed char& value, char delimiter) -> result {
        const char fmt[6] = {'%', 'h', 'h', 'd', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(short& value, char delimiter) -> result {
    ...
    auto _read_one(int& value, char delimiter) -> result {
        const char fmt[4] = {'%', 'd', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(unsigned& value, char delimiter) -> result {
        const char fmt[4] = {'%', 'u', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(long& value, char delimiter) -> result {
        const char fmt[5] = {'%', 'l', 'd', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(unsigned long& value, char delimiter) -> result {
        const char fmt[5] = {'%', 'l', 'u', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(long long& value, char delimiter) -> result {
        const char fmt[6] = {'%', 'l', 'l', 'd', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(unsigned long long& value, char delimiter) -> result {
        const char fmt[6] = {'%', 'l', 'l', 'u', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(float& value, char delimiter) -> result {
        const char fmt[4] = {'%', 'f', delimiter, '\0'};
        return _sscanf_one(value, delimiter, fmt);
    }

    auto _read_one(double& value, char delimiter) -> result {
