...
struct matrix
{
	...
	static inline
	matrix _from_hlp(const P* dt, span_size_t sz, _iseq<I...>)
	noexcept {
		return matrix{{
	...
	static inline
	matrix _from_hlp(const matrix<P,M,N,RM,W>& m, _iseq<I...>)
	noexcept {
		return matrix{{
			vect::cast<
				P,(RM?M:N),W,
	...
	inline
	const vector<T, RM?C:R, V> operator [] (int i) const
	noexcept {
...
int rows(const matrix<T, C, R, RM, V>&)
noexcept {
	return R;
}
...
template <typename T, int C, int R, bool RM, bool V>
static constexpr inline
int columns(const matrix<T, C, R, RM, V>&)
noexcept {
	return C;
}
...
static constexpr inline
int dimension(const matrix<T, N, N, RM, V>&)
noexcept {
	return N;
}
...
static constexpr inline
T get_cm(const matrix<T, C, R,false, V>& m, int ci, int ri)
noexcept {
	assert(ci<C && ri<R);
	return m._v[ci][ri];
}
...
template <typename T, int C, int R, bool V>
static constexpr inline
T get_cm(const matrix<T, C, R, true, V>& m, int ci, int ri)
noexcept {
	assert(ci<C && ri<R);
	return m._v[ri][ci];
}
...
template <typename T, int C, int R, bool V>
static constexpr inline
T get_rm(const matrix<T, C, R,false, V>& m, int ri, int ci)
noexcept {
	assert(ci<C && ri<R);
	return m._v[ci][ri];
}
...
template <typename T, int C, int R, bool V>
static constexpr inline
T get_rm(const matrix<T, C, R, true, V>& m, int ri, int ci)
noexcept {
	assert(ci<C && ri<R);
	return m._v[ri][ci];
}
...
template <typename T, int C, int R, bool V>
static inline
void set_cm(matrix<T, C, R,false, V>& m, int ci, int ri, T v)
noexcept {
	assert(ci<C && ri<R);
	m._v[ci][ri] = v;
}
...
template <typename T, int C, int R, bool V>
static inline
void set_cm(matrix<T, C, R, true, V>& m, int ci, int ri, T v)
noexcept {
	assert(ci<C && ri<R);
	m._v[ri][ci] = v;
}
...
template <typename T, int C, int R, bool V>
static inline
void set_rm(matrix<T, C, R,false, V>& m, int ri, int ci, T v)
noexcept {
	assert(ci<C && ri<R);
	m._v[ci][ri] = v;
}
...
template <typename T, int C, int R, bool V>
static inline
void set_rm(matrix<T, C, R, true, V>& m, int ri, int ci, T v)
noexcept {
	assert(ci<C && ri<R);
	m._v[ri][ci] = v;
}
...
static inline
vector<T, C, V>
row(const matrix<T,C,R,RM,V>& m, int i)
noexcept {
...
static inline
vector<T, R, V>
column(const matrix<T,C,R,RM,V>& m, int i)
noexcept {
