...
template <typename T>
static constexpr inline T safe_size_t_cast(T v) noexcept {
    return v;
}
...
static constexpr inline T safe_size_t_cast(F v) noexcept {
...
static constexpr inline std_align_t std_align(T v) noexcept {
    return safe_size_t_cast<std_align_t>(v);
}

// convert to std size type
template <typename T, typename = std::enable_if_t<std::is_integral_v<T>>>
static constexpr inline std_size_t std_size(T v) noexcept {
    return safe_size_t_cast<std_size_t>(v);
}

// convert to span size type
template <typename T, typename = std::enable_if_t<std::is_integral_v<T>>>
static constexpr inline span_size_t span_size(T v) noexcept {
    return safe_size_t_cast<span_size_t>(v);
}

template <typename T>
static constexpr inline span_size_t span_align_of() noexcept {
    return span_size(alignof(T));
}

template <typename T>
static constexpr inline span_size_t span_size_of() noexcept {
    return span_size(sizeof(T));
}

template <typename T, typename S>
static constexpr inline span_size_t span_size_of(S n) noexcept {
