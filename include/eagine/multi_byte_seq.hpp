...
noexcept { return head_code_from_mask(head_code_mask(len)); }

static constexpr inline
always_valid<byte> tail_code(void)
noexcept { return 0x80; }
...
noexcept {
	return	is_valid_head_byte(b, 1)||
		is_valid_head_byte(b, 2)||
		is_valid_head_byte(b, 3)||
		is_valid_head_byte(b, 4)||
		is_valid_head_byte(b, 5)||
		is_valid_head_byte(b, 6);
}
...
noexcept { return do_decode_sequence_length(byte(seq.value(0)[0])); }
...
noexcept { return do_decode_code_point(src, decode_sequence_length(src)); }
