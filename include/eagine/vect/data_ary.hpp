...
struct _ary_data {
    ...
    auto operator=(_ary_data&&) noexcept(std::is_nothrow_move_constructible_v<T>)
      -> _ary_data& = default;
    ...
    friend auto operator+(const _ary_data& a, const _ary_data& b) noexcept {
        _ary_data c{};
        for(int i = 0; i < N; ++i) {
    ...
    friend auto operator+(const _ary_data& a, T b) noexcept {
        _ary_data c{};
        for(int i = 0; i < N; ++i) {
            c._v[i] = a._v[i] + b;
        }
