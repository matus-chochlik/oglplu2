...
public:
    base_wrapper(eagine::memory::buffer& data,
...
public:
    template <typename... P>
    generator_wrapper(eagine::memory::buffer& tmp_buf,
      const std::array<const vertex_attrib_and_location, N>& vaals,
      P&&... p)
      : wrapper<N>(eagine::identity<Generator>(),
	  tmp_buf,
	  vaals,
	  std::forward<P>(p)...) {
    }
