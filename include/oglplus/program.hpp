...
struct program_ops {
    ...
    static outcome<void> report_program_link_error(program_name prog) noexcept;
    ...
    static outcome<void> program_binary(
      program_name prog, GLenum format, const_memory_block binary) noexcept;
#endif

    static outcome<void> get_program_iv(
      program_name prog, program_parameter param, span<GLint> values) noexcept;
    ...
    static outcome<GLsizei> get_active_attrib(program_name prog,
      GLuint index,
      span<char> name,
      GLint& size,
    ...
    static outcome<GLsizei> get_active_uniform(program_name prog,
      GLuint index,
      span<char> name,
      GLint& size,
      sl_data_type& type) noexcept;
...
// obj_dsa_ops
template <>
struct obj_dsa_ops<tag::program> : obj_zero_dsa_ops<tag::program> {
    typedef oper::program_ops _ops;

    using obj_zero_dsa_ops<tag::program>::obj_zero_dsa_ops;

    outcome<obj_dsa_ops&> attach(shader_name shdr) noexcept {
	return {_ops::attach_shader(*this, shdr), *this};
    }

    outcome<obj_dsa_ops&> detach(shader_name shdr) noexcept {
...
#if defined(GL_VERSION_4_1)
    outcome<obj_dsa_ops&> binary_retrievable_hint(boolean value) noexcept {
	return {_ops::program_binary_retrievable_hint(*this, value), *this};
    }

    outcome<obj_dsa_ops&> separable(boolean value) noexcept {
    ...
    outcome<obj_dsa_ops&> binary(GLenum format, memory_block dest) noexcept {
	return {_ops::program_binary(*this, format, dest), *this};
    }
#endif
    ...
    outcome<GLsizei> get_active_attrib(
      GLuint index, span<char> name, GLint& size, sl_data_type& type) const
      noexcept {
    ...
    outcome<GLsizei> get_active_uniform(
      GLuint index, span<char> name, GLint& size, sl_data_type& type) const
      noexcept {
...
static const object_zero_and_ops<tag::program> no_program = {};
