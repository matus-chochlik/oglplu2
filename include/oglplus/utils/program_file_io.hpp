...
inline void
write_and_pad_shader_source_header(std::ostream& output,
...
inline void
write_shader_source(std::ostream& output,
  shader_source_header& header,
  ...
  span_size_t& spos) {
    ...
    output.write(reinterpret_cast<const char*>(source_text.data()),
...
inline void
write_and_pad_program_source_header(std::ostream& output,
  ...
  span_size_t& spos) {
    ...
    header.shader_sources.reset(hdraddr + std::ptrdiff_t(size),
	...
	output.write(
	  reinterpret_cast<const char*>(&offs), std::streamsize(sizeof(offs)));
	spos += sizeof(offs);
	done += sizeof(offs);
