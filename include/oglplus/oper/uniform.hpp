...
struct uniform_ops {
    ...
    static inline outcome<void> _uniform_vm(prog_var_loc<tag::uniform, D> loc,
      const X& x,
      std::false_type,
    ...
    static inline outcome<void> _uniform_vm(prog_var_loc<tag::uniform, D> loc,
      ...
      std::true_type,
      std::false_type) noexcept {
	return oglplus::prog_var_get_set_ops<tag::uniform,
    ...
    template <bool D,
      ...
      typename = std::enable_if_t<
	is_gl_data_type_v<
	  X> || is_known_vector_type_v<X> || is_known_matrix_type_v<X>>>
    static inline outcome<void> uniform(
