...
using deferred_error_handler =
  deferred_handler<error_info, gl_error_handling_policy>;
...
#define OGLPLUS_VERIFY(GLFUNC, ERROR_INFO, SEVERITY) \
    OGLPLUS_VERIFY_STR(#GLFUNC, ERROR_INFO, SEVERITY)

#define OGLPLUS_VERIFY_SIMPLE(GLFUNC, SEVERITY) \
    OGLPLUS_VERIFY(GLFUNC, no_info(), SEVERITY)

#define OGLPLUS_REPORT_ERROR_STR(                \
  GLFUNC_NAME, ERROR_CODE, ERROR_INFO, SEVERITY) \
    OGLPLUS_RETURN_HANDLER(                      \
      ERROR_CODE, ERROR_INFO.gl_function_name(GLFUNC_NAME), SEVERITY)
