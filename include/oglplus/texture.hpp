...
struct object_subtype<oglplus::tag::texture> {
    using type = oglplus::texture_target;
};
...
using texture_target_only = object_target_only<tag::texture>;
using texture_name_only = object_name_only<tag::texture>;
using texture_name_and_target = object_name_and_target<tag::texture>;

static inline texture_target_only wrap_object_name_or_target(
...
#else
    using _tnt = texture_target_only;
#endif
